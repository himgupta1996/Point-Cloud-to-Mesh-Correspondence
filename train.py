...
#TODO: should tau be 0.07 or 0.01?
def NTcrossentropy(vtx_feature, pts_feature, corr, tau=0.07):
    ...
    yij = torch.diag(torch.matmul(corr_vtx_feature, corr_pts_feature.T)/tau).view(-1, 1)
    yik = torch.logsumexp(torch.matmul(corr_vtx_feature, pts_feature.T)/tau, dim = 1, keepdim=True)
    
    loss = torch.sum(-1*yij + yik)
    return loss
...
def fit_rotation(vtx, pts, vtx_feature, pts_feature, corrmask):
    ...
    similarity_matrix = torch.matmul(vtx_feature, pts_feature.T)[corrmask>0.5]
    _, ni  = torch.max(similarity_matrix, 1)
    
    vtx_corresponding_points = vtx[corrmask>0.5]
    pts_corresponding_points = pts[ni]
    
    vtx_corresponding_points = vtx_corresponding_points-torch.mean(vtx_corresponding_points, 0)
    pts_corresponding_points = pts_corresponding_points-torch.mean(pts_corresponding_points, 0)
    
    scale = torch.pow((torch.norm(vtx_corresponding_points)**2)/(torch.norm(pts_corresponding_points)**2), 1/4)
    
    vtx_corresponding_points_scl = vtx_corresponding_points/scale
    pts_corresponding_points_scl = pts_corresponding_points*scale
    
    S = torch.matmul(vtx_corresponding_points_scl.T, pts_corresponding_points_scl)
    u, s, v = torch.svd(S)
